...
const RPL_NEIGHBOR_TABLE_SIZE: usize = 16;

#[derive(Debug, Default)]
pub struct RplNeighborTable {
    pub(crate) neighbors: [RplNeighborEntry; RPL_NEIGHBOR_TABLE_SIZE],
}
...
impl RplNeighborTable {
    ...
    /// Check if there is place in the neighbor table.
    fn has_free_entry(&self) -> bool {
        self.neighbors
            .iter()
            .any(|n| matches!(n, RplNeighborEntry::Empty))
    }
    ...
    /// Check if the neighbor table contains a neighbor with a specified link-layer address.
    pub(crate) fn has_neighbor(&self, addr: &HardwareAddress) -> bool {
        self.neighbors.iter().any(|neighbor| match neighbor {
            RplNeighborEntry::Neighbor((RplNeighbor { ll_addr, .. }, _)) if ll_addr == addr => true,
            _ => false,
        })
    }
    ...
    pub(crate) fn remove_neighbor(&mut self, addr: Ipv6Address) {
        for n in self.neighbors.iter_mut() {
            match n {
                RplNeighborEntry::Neighbor((RplNeighbor { ip_addr, .. }, _))
                    if *ip_addr == addr =>
                {
