    ...
    DeviceT: for<'d> Device<'d>,
{
    ...
    pub fn poll(&mut self, timestamp: Instant) -> Result<bool> {
        ...
        while !self.out_fragments.is_empty() {
            ...
            let (len, buffer) = self.out_fragments.dequeue_one()?;
    ...
    fn socket_ingress(&mut self) -> bool {
        ...
        while let Some((rx_token, tx_token)) = device.receive() {
            if let Err(err) = rx_token.consume(inner.now, |frame| match inner.caps.medium {
                ...
                #[cfg(feature = "medium-ieee802154")]
                Medium::Ieee802154 => {
                    match inner.process_ieee802154(sockets, &frame, sixlowpan_fragments) {
                        Ok(response) => {
                            processed_any = true;
                            if let Some(packet) = response {
                                if let Err(err) =
        ...
        for item in sockets.iter_mut() {
            ...
            macro_rules! respond {
                ($inner:expr, $response:expr) => {{
                    ...
                    #[cfg(feature = "proto-sixlowpan")]
                    {
                        device_result = $inner.dispatch_ip(tx_token, response, Some(out_fragments));
                    }
                    #[cfg(not(feature = "proto-sixlowpan"))]
                    {
                        device_result = $inner.dispatch_ip(tx_token, response, None);
                    }
                    device_result
                }};
            }
...
impl<'a> InterfaceInner<'a> {
    ...
    fn process_sixlowpan<'output, 'payload: 'output, T: AsRef<[u8]> + ?Sized>(
        ...
        fragments
            .remove_when(|frag| Ok(self.now - frag.start_time()? > Duration::from_secs(60)))?;
        ...
        let payload = match SixlowpanPacket::dispatch(payload)? {
            SixlowpanPacket::FragmentHeader => {
                ...
                let frag = SixlowpanFragPacket::new_checked(payload)?;
                ...
                if frag.is_first_fragment() {
                    ...
                    let iphc = SixlowpanIphcPacket::new_checked(frag.payload())?;
                        ...
                        SixlowpanNextHeader::Compressed => {
                            match SixlowpanNhcPacket::dispatch(iphc.payload())? {
                                ...
                                SixlowpanNhcPacket::UdpHeader => {
                                    let udp_packet =
                                        SixlowpanUdpNhcPacket::new_checked(iphc.payload())?;
                    ...
                    fragments.reserve_with_key(&key)?.start(
                ...
                let frags = fragments.get_packet_assembler_mut(&key)?;

                // Check if 60 seconds have passed since the start of the first fragment.
                if self.now - frags.start_time()? > Duration::from_secs(60) {
                    frags.mark_discarded();
                    return Err(Error::ReassemblyTimeout);
                }
                ...
                match frags.add(frag.payload(), offset, self.now) {
                    Ok(true) => {
                        ...
                        fragments.get_assembled_packet(&key)?
                    }
                    Ok(false) => {
                        return Ok(None);
                    }
                    Err(Error::PacketAssemblerOverlap) => {
                        ...
                        frags.mark_discarded();
                        return Err(Error::PacketAssemblerOverlap);
                    }
                    Err(e) => return Err(e),
                }
            }
        ...
        match iphc_repr.next_header {
            SixlowpanNextHeader::Compressed => {
                match SixlowpanNhcPacket::dispatch(payload)? {
                    ...
                    #[cfg(feature = "socket-udp")]
                    SixlowpanNhcPacket::UdpHeader => {
                        let udp_packet = SixlowpanUdpNhcPacket::new_checked(payload)?;
            ...
            if ipv4_repr.next_header == IpProtocol::Udp && self.hardware_addr.is_some() {
                    ...
                    && udp_packet.dst_port() == DHCP_CLIENT_PORT
                {
                    if let Some(dhcp_socket) = sockets
                        .iter_mut()
                        .find_map(|i| Dhcpv4Socket::downcast(&mut i.socket))
                    {
    ...
    fn dispatch_ieee802154<Tx: TxToken>(
        ...
        if total_size > 125 {
            ...
            while written < total_size {
                out_fragments.enqueue_one_with(|(len, tx_buf)| {
