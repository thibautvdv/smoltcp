    ...
    pub fn resolve(self, ll_address: Option<LlAddress>) -> Result<ipv6::Address> {
        ...
        match self {
            UnresolvedAddress::WithoutContext(mode) => match mode {
                ...
                AddressMode::FullyElided => {
                    ...
                    match ll_address.unwrap() {
                        ...
                        LlAddress::Absent => return Err(Error::Malformed),
                    }
                    Ok(ipv6::Address::from_bytes(&bytes[..]))
                }
            ...
            UnresolvedAddress::WithContext(mode) => match mode {
                ...
                AddressMode::NotSupported => Err(Error::NotSupported),
                _ => Err(Error::Malformed),
            },
            UnresolvedAddress::Reserved => Err(Error::Malformed),
        }
    }
}
...
pub mod iphc {
    ...
    use super::{AddressMode, NextHeader, UnresolvedAddress, DISPATCH_IPHC_HEADER};
    use crate::{
        wire::{ieee802154::Address as LlAddress, ipv6, IpProtocol},
        Error, Result,
    };
    use byteorder::{ByteOrder, NetworkEndian};
...
pub mod nhc {
    ...
    use super::{NextHeader, DISPATCH_EXT_HEADER, DISPATCH_UDP_HEADER};
    use crate::{
        wire::{
